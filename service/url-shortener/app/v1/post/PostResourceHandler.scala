...
object PostResource
{
    ...
    implicit val implicitWrites = new Writes[PostResource]
    {
        ...
        {
            Json.obj("id" -> post.id, "link" -> post.link, "title" -> post.title, "body" -> post.body)
        }
    }
}
...
/**
  ...
  */
class PostResourceHandler @Inject()(routerProvider: Provider[PostRouter], postRepository: PostRepository)(implicit
    ec: ExecutionContext)
{
